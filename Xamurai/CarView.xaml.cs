...
namespace Xamurai
{
	...
	public partial class CarView : ContentView, INotifyPropertyChanged
	{
		...
		public CarView ()
		{
			...
			LongPressCommand = new Command<Car>(async car => await LongPress(car));
		...
		protected override void OnSizeAllocated(double width, double height)
		{
			...
			longPressGrid.IsVisible = DeviceDisplay.MainDisplayInfo.Orientation == DisplayOrientation.Landscape;
		}
		...
		private async Task ToggleCollapse()
		{
			...
			else
			{
				...
				Content.HeightRequest = 65;
				OnPropertyChanged(nameof(IsExpanded));
			}
		}

		private async Task LongPress(Car car)
		{
			if (DeviceDisplay.MainDisplayInfo.Orientation == DisplayOrientation.Landscape)
			{
				...
				if(result)
				{
					...
					MessagingCenter.Send<Car>(car, Hardcoded.MESSAGE_DELETECAR);
				}
			}
		}
	}
